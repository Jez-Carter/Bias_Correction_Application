...
csv_files = glob.glob(os.path.join(f'{data_path}Monthly_75%/', "*"))
...
for file in csv_station_files:
    ...
    station_name = file.split('/')[-1].split('_month.csv')[0]
    df['Station']=station_name
    dfs.append(df)
df_all = pd.concat(dfs)
df_all = df_all.reset_index(drop=True)
df_all = df_all[df_all['Year'].notna()]
df_all = df_all[['Station','Year','Month','Temperature()']]
...
# %% Saving output
df_all_combined.to_csv(f'{outpath}df_all_combined_75_monthly.csv',index=False)
