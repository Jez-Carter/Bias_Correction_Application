...
# %% Plotting the correction for each parameter

ds_climate_coarse_june_stacked['Mean Temperature Bias'] = (ds_climate_coarse_june_stacked['Mean Temperature'] -
                                                    ds_climate_coarse_june_stacked['exp_mean_unbiased'])
ds_climate_coarse_june_stacked['Log Var Temperature Bias'] = (ds_climate_coarse_june_stacked['Log Var Temperature'] -
                                                    ds_climate_coarse_june_stacked['exp_logvar_unbiased'])
...
ds_climate_coarse_june_stacked['Mean Temperature Bias'].unstack().plot.pcolormesh(
        x='glon',
        y='glat',
        ax=axs[0,0],
        cmap='RdBu',
        vmin=-10,
        vmax=10,
        cbar_kwargs = dict(cbar_kwargs, label='Mean Correction Expecation')
    )

ds_climate_coarse_june_stacked['Log Var Temperature Bias'].unstack().plot.pcolormesh(
        x='glon',
        y='glat',
        ax=axs[0,1],
        cmap='RdBu',
        vmin=-1,
        vmax=1,
        cbar_kwargs = dict(cbar_kwargs, label='Log-Var. Correction Expecation')
    )
...
(2*ds_climate_coarse_june_stacked['std_mean_unbiased']).unstack().plot.pcolormesh(
        ...
        vmin=5,
        vmax=10,
        cbar_kwargs = dict(cbar_kwargs, label='Mean Correction $2\sigma$ Uncertainty')
    )
...
(2*ds_climate_coarse_june_stacked['std_logvar_unbiased']).unstack().plot.pcolormesh(
        ...
        # vmin=-0.3,
        # vmax=0.3,
        cbar_kwargs = dict(cbar_kwargs, label='Log-Var. Correction $2\sigma$ Uncertainty')
    )
...
for ax,title in zip(axs[0,:], ['Empirical C.M. Mean - Unbiased Mean Pred.','Empirical C.M. Log-Var. - Unbiased Log-Var. Pred.']):
...
plt.tight_layout()
fig.savefig(f"{results_path}figa05.pdf", dpi=300, bbox_inches="tight")
...
plt.tight_layout()
fig.savefig(f"{results_path}fig19.pdf", dpi=300, bbox_inches="tight")
...
for ax,station,label in zip(axs[:-1],stations[:-1],labels[:-1]):
    ...
    ax.fill_between(
        ds_climate['year'],
        c_corrected.mean(axis=0)
        + 2 * c_corrected.std(axis=0),
        c_corrected.mean(axis=0)
        - 2 * c_corrected.std(axis=0),
        interpolate=True,
        color="k",
        alpha=0.5,
        label="Bias Corrected Output Uncertainty 2$\sigma$",
        linewidth=0.5,
        facecolor="none",
        edgecolor="k",
        linestyle=(0, (5, 2)),
    )
    for corrected_timeseries in c_corrected[::20]:
        ax.plot(
            ds_climate['year'],
...
plt.tight_layout()
fig.savefig(f"{results_path}fig20.pdf", dpi=300, bbox_inches="tight")
