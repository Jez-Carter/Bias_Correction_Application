...
labels = ['$\phi_Y$','$\phi_B$','$\phi_Z$','In-Situ Observations','Climate Model Output']
...
parameters = ['In-Situ Kernel Variance $v_{\phi_Y}$',
...
scenarios_lima = [scenario_ampledata_lima,scenario_sparse_smooth_lima,scenario_sparse_complex_lima]
...
for scenario,values in zip(scenarios_lima,values_list):
    df_lima = az.summary(scenario['mcmc_lima'].posterior,hdi_prob=0.95)
    ...
    # df_lima.insert(0, 'Specified Value', np.array(values)[[0,1,2,3,4,5,7]])
    print(df_lima.to_latex(escape=False))
...
for scenario,scenario_lima,values in zip(scenarios,scenarios_lima,values_list):
    ...
    df_conc.insert(0, 'Specified Value', np.array(values)[[0,1,2,3,4,5,7]])
...
posterior_keys = ['kern_var','lengthscale','mean',
                  ...
                  'onoise']
titles = ['a. Kernel Variance $v_{\phi_Y}$',
...
# scenario_sparse_smooth = np.load(f'{inpath}scenario_sparse_smooth.npy',allow_pickle='TRUE').item()
# scenario_sparse_smooth_lima = np.load(f'{inpath}scenario_sparse_smooth_lima.npy',allow_pickle='TRUE').item()
# scenario = scenario_sparse_smooth
# scenario_lima = scenario_sparse_smooth_lima
...
axs[0,0].annotate('a. Scenario 1 (1 Process)',xy=(0.01,1.01),xycoords='axes fraction')
axs[0,1].annotate('b. Scenario 1 (2 Process)',xy=(0.01,1.01),xycoords='axes fraction')
axs[1,0].annotate('c. Scenario 2 (1 Process)',xy=(0.01,1.01),xycoords='axes fraction')
axs[1,1].annotate('d. Scenario 2 (2 Process)',xy=(0.01,1.01),xycoords='axes fraction')
axs[2,0].annotate('e. Scenario 3 (1 Process)',xy=(0.01,1.01),xycoords='axes fraction')
axs[2,1].annotate('f. Scenario 3 (2 Process)',xy=(0.01,1.01),xycoords='axes fraction')
...
plt.tight_layout()
plt.show()
fig.savefig(f'{out_path}fig11.png',dpi=300,bbox_inches='tight')
