...
#### Random ####
obs_sample = np.random.choice(ds_stacked.X.size, size=20, replace=False)
da_obs_stacked = ds_stacked.isel(X=obs_sample).Y
...
axs[0].scatter(da_obs_stacked.dropna('X').X1, da_obs_stacked.dropna('X').X2, s=30, marker='o', c='w',edgecolor='k')
axs[2].scatter(da_climate_stacked.X1, da_climate_stacked.X2, s=30, marker='+',c='k')
# axs[2].grid(xdata=da_climate_stacked.X1, ydata = da_climate_stacked.X2)
sepX1 = da_climate.X1[1]-da_climate.X1[0]
sepX2 = da_climate.X2[1]-da_climate.X2[0]
axs[2].set_xticks(da_climate.X1+sepX1/2, minor=True)
axs[2].set_yticks(da_climate.X2+sepX2/2, minor=True)
axs[2].xaxis.grid(True, which='minor',linestyle='--')
axs[2].yaxis.grid(True, which='minor',linestyle='--')
plt.tight_layout()
# fig.savefig('ideal.png', format='png', dpi=1200)

# %%
# plt.rcParams['text.usetex'] = True

kern = 1 * kernels.ExpSquared(10)
...
fig, axs = plt.subplots(1,3,figsize=(15,4))
axs[0].plot(x1,kern(x0,x1)[0],label='kernel variance=1, lengthscale=10')
axs[0].set_xlabel(r"$\mathbf{s-s'}$")
axs[0].set_ylabel(r"$RBF(\mathbf{s-s'})$")

plot1 = axs[1].pcolormesh(X[0],X[1],kern(x0,x1))
axs[1].set_xlabel(r"$\mathbf{s}$")
axs[1].set_ylabel(r"$\mathbf{s'}$")
fig.colorbar(plot1, ax=axs[1],label=r"$RBF(\mathbf{s-s'})$")

ds['Y'].plot.contourf(x='X1',y='X2',levels=levels,center=0,ax=axs[2],label='huh')
axs[0].legend()
plt.tight_layout()
...
ax.scatter(da_climate_stacked.X1, da_climate_stacked.X2, s=30, marker='+',c='k')
...
for i,var,title,levels in zip(range(2, 4), variables, titles,[truth_levels,uncertainty_levels]):
    ...
    plt.title(title)
fig.savefig('2d_truth_pred.png', format='png', dpi=1200)
...
for i,var,title,levels in zip(range(2, 4), variables, titles,[truth_levels,uncertainty_levels]):
    ...
    plt.title(title)
fig.savefig('2d_bias_pred.png', format='png', dpi=1200)
